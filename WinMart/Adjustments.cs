    ...
    public partial class Adjustments : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (cbAction.Text == "")
                {
                    MessageBox.Show("Vui lòng chọn hành động để thêm hoặc bớt", "WinMart", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    cbAction.Focus();
                    return;
                }
                ...
                if (txtQty.Text == "")
                {
                    MessageBox.Show("Vui lòng nhập số lượng để thêm hoặc bớt.", "WinMart", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    txtQty.Focus();
                    return;
                }
                ...
                if (txtRemark.Text == "")
                {
                    MessageBox.Show("Cần lý do để điều chỉnh.", "WinMart", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    txtRemark.Focus();
                    return;
                }
                ...
                {
                    MessageBox.Show("Số lượng hàng trong kho phải lớn hơn số lượng điều chỉnh.", "WinMart", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                if (cbAction.Text == "Bớt")
                {
                    dbcon.ExecuteQuery("UPDATE tbProduct SET qty = (qty - " + int.Parse(txtQty.Text) + ") WHERE pcode LIKE '" + lblPcode.Text + "'");
                }
                else if (cbAction.Text == "Thêm")
                {
                    dbcon.ExecuteQuery("UPDATE tbProduct SET qty = (qty + " + int.Parse(txtQty.Text) + ") WHERE pcode LIKE '" + lblPcode.Text + "'");
                }
                ...
                MessageBox.Show("Thay đổi thành công.", "Hoàn thành", MessageBoxButtons.OK, MessageBoxIcon.Information);
                LoadStock();
                Clear();
            ...
            {
                MessageBox.Show(ex.Message, "WinMart", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }

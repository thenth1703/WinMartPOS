...
namespace WinMart
{
    public partial class Supplier : Form
    {
        ...
        private void dgvSupplier_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (MessageBox.Show("Bạn có chắc chắn muốn xoá bản ghi này?", "Xoá bản ghi", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    cn.Open();
                    cm = new SqlCommand("Delete from tbSupplier where id like " + dgvSupplier.Rows[e.RowIndex].Cells[1].Value.ToString() + "* ", cn);
                    cm.ExecuteNonQuery();
                    cn.Close();
                    MessageBox.Show("Xoá thành công", "Xoá bản ghi", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            LoadSupplier();
        }

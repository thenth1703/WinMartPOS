...
namespace WinMart
{
    public partial class Record : Form
    {
        ...
        public void LoadTopSelling()
        {
            ...
            else if (cbTopSell.Text == "Sắp xếp theo tổng tiền")
            {
            ...
            {
                i++;
                dgvTopSelling.Rows.Add(i, dr["pcode"].ToString(), dr["pdesc"].ToString(), dr["qty"].ToString(), double.Parse(dr["total"].ToString()).ToString("#,##0"));
            }
            dr.Close();
            cn.Close();
        }
        public void LoadSoldItems()
        {
            try
            {
                ...
                {
                    i++;
                    dgvSoldItems.Rows.Add(i, dr["pcode"].ToString(), dr["pdesc"].ToString(), double.Parse(dr["price"].ToString()).ToString("#,##0"), dr["qty"].ToString(), dr["disc"].ToString(), double.Parse(dr["total"].ToString()).ToString("#,##0"));
                }
                dr.Close();
                cn.Close();
                ...
                lblTotal.Text = double.Parse(cm.ExecuteScalar().ToString()).ToString("#,##0");
                cn.Close();
            }
            catch (Exception ex)
            {
        ...
        private void btnLoadTopSell_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Chọn kiểu sắp xếp.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                cbTopSell.Focus();
                return;
            }
            LoadTopSelling();
        ...
        private void btnPrintSoldItems_Click(object sender, EventArgs e)
        {
            ...
            string param = "Từ ngày : " + dtFromSoldItems.Value.ToString() + " Đến : " + dtToSoldItems.Value.ToString();

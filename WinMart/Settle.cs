    ...
    public partial class Settle : Form
    {
        ...
        private void btnEnter_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Không đủ số tiền, Vui lòng nhập số tiền chính xác!", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                else
                {
                    ...
                    MessageBox.Show("Thanh toán đã được lưu thành công!", "WinMart", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    cashier.GetTranNo();
                    cashier.LoadCart();
                    this.Dispose();
                }
        ...
        private void txtCash_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                txtChange.Text = charge.ToString("#,##0");
            }
            catch (Exception)
            {
                txtChange.Text = "0";
